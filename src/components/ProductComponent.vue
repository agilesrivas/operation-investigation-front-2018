        ...
        <table id="table" class="__table __full-width-perc __text-center __table-bordered __table-hoverrable">
          ...
          <tbody>
            <tr v-for="(tock, index) in stocks" :key="index">
              ...
              <td> {{ tock.provideer.leadtime }} </td>
              <td> {{ tock.category.name }} </td>
              <td> {{ tock.provideer.name }} </td>
              <td>
...
export default {
  ...
  methods: {
    create() {
      if ( this.product.name.trim() && this.product.amount >= 0 && this.product.cost >= 0  && this.product.code.trim() &&  this.product.category && this.product.provideer ) {
